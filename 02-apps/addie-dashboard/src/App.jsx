...
import {
  STORAGE_KEY, SETTINGS_KEY, DEFAULT_CONSULTANT, blankProject, DEFAULT_PANELS, PANEL_LABELS,
  migrateProject, STATUS_COLOR, STATUS_OPTIONS, RAG_DOT, RAG_BADGE, pct, nowStr, todayISO,
  loadState, saveState, loadConsultant, saveConsultant,
} from "./lib/dashboard";
import { buildClientHTML, buildShareURL } from "./lib/report";
import { buildCSVRows, buildExcelSheets, csvText, exportFileName } from "./lib/exporters";
...
function exportCSV(proj) {
  const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(new Blob([csvText(buildCSVRows(proj))], { type: "text/csv" })), download: exportFileName(proj, ".csv") });
  a.click();
}

function exportExcel(proj) {
  const wb = XLSX.utils.book_new();
  Object.entries(buildExcelSheets(proj)).forEach(([name, data]) => XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(data), name));
  XLSX.writeFile(wb, exportFileName(proj, ".xlsx"));
}
...
export default function App() {
  const [state, setState] = useState(() => {
    ...
    if (saved) saved.projects = saved.projects.map(migrateProject);
  ...
  useEffect(() => {
    loadFromSupabase().then(cloudState => {
      if (cloudState) {
        cloudState.projects = cloudState.projects.map(migrateProject);
        setState(cloudState);
